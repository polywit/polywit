...
class TestValidator:
    ...
    @patch('halo.Halo.succeed')
    def test_preprocess_fails_on_file_processor_error(self, mock_succeed):
        self.file_processor.preprocess.side_effect = FileProcessorError()
        self.witness_processor.preprocess.return_value = None
        validator = self.construct_validator()
        with pytest.raises(FileProcessorError):
            validator.preprocess()
        assert self.file_processor.preprocess.call_count == 1
        assert mock_succeed.call_count == 0

    @patch('halo.Halo.succeed')
    def test_preprocess_fails_on_witness_processor_error(self, mock_succeed):
        self.file_processor.preprocess.return_value = None
        ...
        with pytest.raises(WitnessProcessorError):
            validator.preprocess()
        assert self.file_processor.preprocess.call_count == 1
        assert self.witness_processor.preprocess.call_count == 1
    ...
    @patch('halo.Halo.succeed')
    def test_extract_assumption_fails_on_extract_position_type_map(self, mock_succeed):
        self.file_processor.extract_position_type_map.side_effect = FileProcessorError()
        validator = self.construct_validator()
        with pytest.raises(FileProcessorError):
            validator.extract_assumptions()
        assert self.file_processor.extract_position_type_map.call_count == 1
        assert self.witness_processor.extract_assumptions.call_count == 0
        assert mock_succeed.call_count == 0

    @patch('halo.Halo.succeed')
    def test_extract_assumption_fails_on_extract_assumptions(self, mock_succeed):
        ...
        with pytest.raises(WitnessProcessorError):
            validator.extract_assumptions()
        assert self.file_processor.extract_position_type_map.call_count == 1
        assert self.witness_processor.extract_assumptions.call_count == 1
        assert mock_succeed.call_count == 1
    ...
    @patch('halo.Halo.succeed')
    def test_execute_test_harness_fails_when_cannot_build_harness(self, mock_succeed):
        ...
        assert self.test_harness.build_test_harness.call_count == 1
        assert mock_succeed.call_count == 0

    @patch('halo.Halo.succeed')
    def test_execute_test_harness_fails_when_cannot_run_harness(self, mock_succeed):
        self.test_harness.build_test_harness.return_value = None
        ...
        assert self.test_harness.build_test_harness.call_count == 1
        assert self.test_harness.run_test_harness.call_count == 1
        assert mock_succeed.call_count == 1
